...
fn generate_expr(expr: &MathExpr) -> syn::Result<TokenStream2> {
    match expr {
        ...
        MathExpr::Ident(id) => {
            ...
            match name.as_str() {
                "pi" | "Pi" | "PI" => Ok(quote! { ::symplex::default_context().pi() }),
                "E" => Ok(quote! { ::symplex::default_context().e() }),
                "I" => Ok(quote! { ::symplex::default_context().i_unit() }),
                "oo" | "inf" => Ok(quote! { ::symplex::default_context().infinity() }),
                _ => Ok(quote! { (&#id) }),
            }
        }
            ...
            match op {
                ...
                BinOp::Div => {
                    if let (Some(p), Some(q)) = (lhs.as_int(), rhs.as_int()) {
                        ...
                        return Ok(quote! { ::symplex::default_context().rational(#p, #q) });
                    }
                    ...
                    if let MathExpr::Neg(inner_lhs) = lhs.as_ref() {
                        if let (Some(p), Some(q)) = (inner_lhs.as_int(), rhs.as_int()) {
                            ...
                            let neg_p = -p;
                            return Ok(quote! { ::symplex::default_context().rational(#neg_p, #q) });
                        }
                    }
                    let lhs_code = generate_expr(lhs)?;
                    let rhs_code = generate_expr(rhs)?;
...
fn generate_dim_expr(expr: &MathExpr) -> syn::Result<TokenStream2> {
    match expr {
        ...
        MathExpr::Ident(id) => {
            ...
            match name.as_str() {
                "pi" | "Pi" | "PI" => Ok(quote! {
                    ::symplex::units::Dimensionless::from_ex(::symplex::default_context().pi()).as_qty()
                }),
                "E" => Ok(quote! {
                    ::symplex::units::Dimensionless::from_ex(::symplex::default_context().e()).as_qty()
                }),
...
fn generate_expr_as_ex(expr: &MathExpr) -> syn::Result<TokenStream2> {
    match expr {
        MathExpr::Int(n, _) => Ok(quote! { ::symplex::default_context().int(#n) }),
        MathExpr::Neg(inner) => {
            if let Some(n) = inner.as_int() {
                let neg_n = -n;
                Ok(quote! { ::symplex::default_context().int(#neg_n) })
            } else {
