...
impl<'a> Emitter<'a> {
    ...
    fn lower_node(&mut self, id: ExprId) -> Result<(), SymplexError> {
        ...
        match arena.node(id).clone() {
            ...
            ExprNode::EulerGamma => self.emit(Instruction::PushConst(
                crate::output::codegen::numeric_rt::EULER_GAMMA_F64,
            )),
            ExprNode::Catalan => self.emit(Instruction::PushConst(
                crate::output::codegen::numeric_rt::CATALAN_F64,
            )),
            ExprNode::GoldenRatio => self.emit(Instruction::PushConst(
                crate::output::codegen::numeric_rt::GOLDEN_RATIO_F64,
            )),
