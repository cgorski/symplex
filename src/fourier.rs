...
pub(crate) fn fourier_series(arena: &mut Arena, expr: ExprId, var: ExprId, n_terms: u32) -> ExprId {
    ...
    // a₀ = (1/π) ∫_{-π}^{π} f(x) dx
    let integral_f = crate::integrate::integrate(arena, expr, var);
    let f_at_pi = crate::subs::subs(arena, integral_f, var, pi);
    let f_at_neg_pi = crate::subs::subs(arena, integral_f, var, neg_pi);
    ...
    for n in 1..=n_terms {
        ...
        let integral_cos = crate::integrate::integrate(arena, f_cos, var);
        let at_pi = crate::subs::subs(arena, integral_cos, var, pi);
        let at_neg_pi = crate::subs::subs(arena, integral_cos, var, neg_pi);
        ...
        let integral_sin = crate::integrate::integrate(arena, f_sin, var);
        let at_pi_s = crate::subs::subs(arena, integral_sin, var, pi);
        let at_neg_pi_s = crate::subs::subs(arena, integral_sin, var, neg_pi);
