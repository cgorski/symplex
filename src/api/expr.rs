...
impl<S: Sort> Expr<S> {
    ...
    pub fn simplify_trace(&self) -> (Expr<S>, Vec<crate::transforms::pattern::Step>) {
        ...
        if inner.cached_rules.is_none() {
            inner.cached_rules = Some(crate::transforms::pattern::basic_rules(&mut inner.arena));
        }
    ...
    pub fn full_simplify_trace(&self) -> (Expr<S>, Vec<crate::transforms::pattern::Step>) {
        ...
        let (result_id, steps) = {
            ...
            crate::simplify::simplify_engine::full_simplify_trace(&mut guard.arena, self.id)
        };
        (self.wrap(result_id), steps)
    }
