...
mod tests {
    ...
    fn fps_exp_truncate_matches_maclaurin() {
        ...
        let maclaurin = crate::calculus::series::series(&mut a, exp_x, x, zero, 5).unwrap();

        // Both should produce the same expanded polynomial
        let t_expanded = crate::transforms::expand::expand(&mut a, truncated);
        let t_evaled = crate::transforms::eval::eval(&mut a, t_expanded);
        let m_expanded = crate::transforms::expand::expand(&mut a, maclaurin);
        let m_evaled = crate::transforms::eval::eval(&mut a, m_expanded);
