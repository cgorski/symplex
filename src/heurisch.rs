...
fn heurisch_attempt(
    ...
    let comp_derivs: Vec<ExprId> = components
        .iter()
        .map(|&c| crate::diff::diff(arena, c, var))
        .collect();
    ...
    let candidate_deriv = crate::diff::diff(arena, candidate, var);
...
fn build_candidate(
    ...
    crate::subs::subs_map(arena, candidate_in_v, &back_subs)
}
...
mod tests {
    ...
    fn heurisch_exp_x() {
        ...
        if let Some(r) = result {
            ...
            let deriv = crate::diff::diff(&mut a, r, x);
