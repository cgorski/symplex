    ...
    for n in 1..=big_n {
        ...
        for nu in 0..n {
            ...
            for m in 0..=m_max {
                ...
                let val = crate::eval::eval(arena, val);
                delta[m][n][nu] = val;
            }
        }
    ...
    match node {
        ExprNode::Derivative(body, wrt) => {
            ...
            if wrt == var {
                ...
                let f_plus = crate::subs::subs(arena, body, var, x_plus);

                // f(x - h/2)
                let x_minus = arena.sub(var, half_h);
                let f_minus = crate::subs::subs(arena, body, var, x_minus);
...
mod tests {
    ...
    fn apply_finite_diff_quadratic_exact() {
        ...
        let result_eval = crate::eval::eval(&mut a, result);
