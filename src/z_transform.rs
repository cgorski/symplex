...
fn contains_var(arena: &Arena, expr: ExprId, var: ExprId) -> bool {
    crate::walk::contains(arena, expr, var)
}
...
fn extract_linear_coeff(arena: &mut Arena, expr: ExprId, n_var: ExprId) -> Option<ExprId> {
    ...
    let poly = crate::polybridge::expr_to_poly(arena, expr, n_var)?;
    if poly.degree()? != 1 {
        return None;
    }
    let c0 = poly.coeff(0);
...
fn do_inverse(
    ...
    let decomposed = crate::apart::apart(arena, expr, z_var);
    if decomposed != expr {
...
fn try_table_inverse(
    ...
    let denom_poly = crate::polybridge::expr_to_poly(arena, denom, z_var)?;
...
fn inverse_degree1(
    ...
    let numer_poly = crate::polybridge::expr_to_poly(arena, numer, z_var)?;
