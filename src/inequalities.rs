...
pub(crate) fn solve_inequality(
    ...
    for &root in &roots {
        let evaled = crate::eval::eval(arena, root);
...
fn solve_no_roots(
    ...
    let zero = arena.zero;
    let val = crate::subs::subs(arena, expr, var, zero);
    let val_eval = crate::eval::eval(arena, val);
...
fn sign_matches(
    ...
    let substituted = crate::subs::subs(arena, expr, var, test_expr);
    let evaluated = crate::eval::eval(arena, substituted);
