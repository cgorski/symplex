...
#[test]
fn evalf_finite_sum() {
    let __vars_ctx = symplex::default_context().clone(); symplex::syms!(__vars_ctx; k);
    let s = Ex::symbolic_sum(&k, &k, &symplex::default_context().int(1), &symplex::default_context().int(10));
...
fn evalf_sum_of_squares() {
    let __vars_ctx = symplex::default_context().clone(); symplex::syms!(__vars_ctx; k);
    let body = k.powi(2);
    let s = Ex::symbolic_sum(&body, &k, &symplex::default_context().int(1), &symplex::default_context().int(5));
    let result = s.eval_f64().unwrap();
    // 1 + 4 + 9 + 16 + 25 = 55
    assert!(
        (result - 55.0).abs() < 1e-10,
...
fn evalf_finite_product() {
    let __vars_ctx = symplex::default_context().clone(); symplex::syms!(__vars_ctx; k);
    let p = Ex::symbolic_product(&k, &k, &symplex::default_context().int(1), &symplex::default_context().int(5));
...
fn evalf_binomial_5_2() {
    // C(5,2) = 10
    let result = symplex::default_context().int(5)
        .binomial(&symplex::default_context().int(2))
        .eval_f64()
        .unwrap();
    assert!(
...
fn evalf_binomial_10_3() {
    // C(10,3) = 120
    let result = symplex::default_context().int(10)
        .binomial(&symplex::default_context().int(3))
        .eval_f64()
        .unwrap();
    assert!(
...
fn evalf_piecewise_true_branch() {
    ...
    let val = symplex::default_context().int(42);
    let cond = symplex::default_context().int(1).gt(&symplex::default_context().int(0)); // 1 > 0 → True after eval
...
fn piecewise_with_else_branch() {
    ...
    let val = symplex::default_context().int(42);
    let cond = symplex::default_context().int(1).gt(&symplex::default_context().int(0)); // 1 > 0 → True
    let pw = Ex::piecewise(&[(&val, &cond)]);
...
fn piecewise_all_false_returns_error() {
    ...
    let cond_f1 = symplex::default_context().int(0).gt(&symplex::default_context().int(1)); // 0 > 1 → False
    let cond_f2 = symplex::default_context().int(0).gt(&symplex::default_context().int(1)); // 0 > 1 → False
    let pw = Ex::piecewise(&[
        (&symplex::default_context().int(1), &cond_f1),
        (&symplex::default_context().int(2), &cond_f2),
    ]);
    let result = pw.eval_f64();
...
fn piecewise_first_true_wins() {
    ...
    let cond_t1 = symplex::default_context().int(1).gt(&symplex::default_context().int(0)); // 1 > 0 → True
    let cond_t2 = symplex::default_context().int(1).gt(&symplex::default_context().int(0)); // 1 > 0 → True
    let pw = Ex::piecewise(&[
        (&symplex::default_context().int(1), &cond_t1),
        (&symplex::default_context().int(2), &cond_t2),
    ]);
    let result = pw.eval_f64();
    assert!(result.is_ok());
