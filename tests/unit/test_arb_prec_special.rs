...
fn gamma_integer_high_precision() {
    ...
    let result = ctx.int(5).gamma().eval_decimal(30).unwrap();
    assert!(
...
fn gamma_small_integer_table() {
    ...
    for &(n, expected) in factorials {
        let result_str = ctx.int(n).gamma().eval_decimal(15).unwrap();
...
fn lambertw_basic_solve() {
    ...
    let w0 = ctx.int(0).lambertw().eval();
...
fn lambertw_neg_one_over_e() {
    ...
    let neg_one_over_e = -ctx.e().powi(-1);
...
fn erf_at_zero_high_precision() {
    ...
    let result = ctx.int(0).erf().eval_decimal(30).unwrap();
    let val: f64 = result.parse().unwrap();
    assert!(
        val.abs() < 1e-20,
...
fn erf_symmetry() {
    ...
    let pos = ctx.int(1).erf().eval_decimal(20).unwrap();
    let neg = ctx.int(-1).erf().eval_decimal(20).unwrap();
...
fn erf_at_one_matches_known_value() {
    ...
    let result = ctx.int(1).erf().eval_decimal(15).unwrap();
    assert!(
...
fn erf_at_two() {
    ...
    let result = ctx.int(2).erf().eval_decimal(15).unwrap();
    assert!(
...
fn erf_large_argument() {
    ...
    let result = ctx.int(5).erf().eval_decimal(15).unwrap();
    let val: f64 = result.parse().unwrap();
    assert!(
        (val - 1.0).abs() < 1e-10,
...
fn erfc_at_zero() {
    let ctx = Context::new();
    // erfc(0) = 1
    let result = ctx.int(0).erfc().eval_decimal(15).unwrap();
    let val: f64 = result.parse().unwrap();
    assert!(
        (val - 1.0).abs() < 1e-12,
...
fn beta_symmetry() {
    ...
    let a = ctx.rational(3, 2);
    let b = ctx.rational(5, 2);
