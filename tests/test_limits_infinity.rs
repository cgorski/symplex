...
fn limit_one_over_x_at_infinity() {
    ...
    let expr = 1 / &x;
    let r = expr.limit(&x, &__ctx.infinity());
...
fn limit_constant_at_infinity() {
    let __ctx = Context::new();
    let x = __ctx.symbol("x");
    let r = __ctx.int(5).limit(&x, &__ctx.infinity());
...
fn limit_x_squared_at_infinity() {
    let __ctx = Context::new();
    let x = __ctx.symbol("x");
    let r = x.powi(2).limit(&x, &__ctx.infinity());
    let s = format!("{r}");
...
fn limit_rational_coeffs_at_pos_inf() {
    ...
    let x = __ctx.symbol("x");
    let numer = &__ctx.int(3) * &x.powi(2) + 1;
    let denom = &__ctx.int(-2) * &x + 5;
    let expr = &numer / &denom;
    let result = expr.limit(&x, &__ctx.infinity());
    assert!(!result.has_unevaluated(), "limit should succeed");
    assert_eq!(format!("{result}"), "-oo");
