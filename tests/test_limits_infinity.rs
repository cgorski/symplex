...
fn limit_one_over_x_at_infinity() {
    ...
    let expr = 1 / &x;
    let r = expr.limit(&x, &symplex::default_context().infinity());
...
fn limit_constant_at_infinity() {
    let x = symplex::default_context().symbol("x");
    let r = symplex::default_context().int(5).limit(&x, &symplex::default_context().infinity());
...
fn limit_x_squared_at_infinity() {
    let x = symplex::default_context().symbol("x");
    let r = x.powi(2).limit(&x, &symplex::default_context().infinity());
    let s = format!("{r}");
