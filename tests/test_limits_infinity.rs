...
fn limit_one_over_x_at_infinity() {
    let x = symplex::var("x");
    let expr = 1 / &x;
    let r = expr.limit(&x, &symplex::infinity()).expect("limit should succeed");
...
fn limit_constant_at_infinity() {
    let x = symplex::var("x");
    let r = symplex::int(5).limit(&x, &symplex::infinity()).expect("limit should succeed");
...
fn limit_x_squared_at_infinity() {
    let x = symplex::var("x");
    let r = x.powi(2).limit(&x, &symplex::infinity()).expect("limit should succeed");
    let s = format!("{r}");
...
fn limit_rational_coeffs_at_pos_inf() {
    ...
    let x = symplex::var("x");
    let numer = &symplex::int(3) * &x.powi(2) + 1;
    let denom = &symplex::int(-2) * &x + 5;
    let expr = &numer / &denom;
    let result = expr.limit(&x, &symplex::infinity());
    assert!(result.is_ok(), "limit should succeed");
    assert_eq!(format!("{}", result.unwrap()), "-oo");
