...
fn integrate_1_over_sqrt_1_plus_x2() {
    // ∫ 1/√(1+x²) dx = asinh(x)
    let x = symplex::var("x");
    let integrand = &symplex::int(1) / &(&x.powi(2) + 1).sqrt();
...
fn integrate_1_over_sqrt_4x2_plus_1() {
    ...
    let x = symplex::var("x");
    let integrand = &symplex::int(1) / &(&(&x.powi(2) * &symplex::int(4)) + 1).sqrt();
...
fn integrate_1_over_sqrt_quadratic_with_linear_term() {
    ...
    let x = symplex::var("x");
    let quad = &(&x.powi(2) + &(&symplex::int(2) * &x)) + 5;
    let integrand = &symplex::int(1) / &quad.sqrt();
...
fn integrate_1_over_sqrt_quadratic_neg_a() {
    ...
    let x = symplex::var("x");
    let quad = &symplex::int(3) - &(&x.powi(2) * &symplex::int(2));
    let integrand = &symplex::int(1) / &quad.sqrt();
