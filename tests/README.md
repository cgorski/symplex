...
~360 integration-test source files (~10 000 `#[test]`s) compiled into **29
test binaries**, plus the SymPy oracle described below.  (The crate's inline
unit tests add ~2 800, and `cargo test --doc` runs ~1 040 API doctests plus
the ~240 Rust blocks of `README.md` and the book, wired in `src/doctests.rs`.)  Everything runs with
...
~6.5 min to link and ~13 GB of `target/`; 29 take seconds.  A new
`vNN.rs` group is added per minor release; everything else goes into an
existing group.
...
|---|---|
| `v09.rs` … `v29.rs` → `vNN/vNN_*.rs` | Feature and regression suites of 0.9 → 0.30, one group per minor release (the group number trails the release since 0.15): **v09** `algebraic, calculus_util, matrix, ntheory_discrete, output, special`; **v10** `stats_continuous, stats_discrete, stats_joint`; **v11** `calculus_algebraic_fixes, output_fixes, review_fixes, special_fixes`; **v12** `betainc, numeric_summation, stats_families, stats_transforms`; **v13** `agreement, data_estimation, hypothesis, walkthrough` (the book chapter's numbers); **v14** `multivariate_order, regression, reliability, survival`; **v17** `anova, audit_a/b/c, exact_intervals, pvalues`; **v18** `cox, layout` (the stats placement rules), `loose_ends, poly_fast, roots, samplers`; **v19** `kernel, mnlogit, raters`; **v20** `base, dense, evalf, fits, fp_poly, libfn, matrix_tier, numdist, poly_kernel`; **v21** (0.22) `apart, coverage, integrate, numdist, series, simplify, solve, units` — the regressions of the 0.21 differential audit and tests for public functions no test reached. **v22** (0.23) `evalf, integrate, simplify` — the domain model: principal-branch evaluation, and every rewrite that was right only on the reals or for positive arguments, each pinned at the point where it was wrong; **v23** (0.24) `eval, integrate` — the wrong antiderivatives the Rubi suite (`rubi-harness/`) and `fuzz_integrate` found; **v24** (0.25) `canon, integrate, panics` — the logarithmic part of the rational integrator, memory budgets, panics on caller input; **v25** (0.26) `evalf, radicals` — digits the evaluator can vouch for, canonical radicals; **v26** (0.27) `hypothesis, numdist` — statistics bug hunt round 1; **v27** (0.28) `tails` — far tails of the symbolic distributions; **v28** (0.29) `api, binders, evalf, integrate, perf, stats, subs_matrix` — the known bugs resolved after 0.28 (the integrator's self-check, binder-aware substitution, the evaluator's error bounds, panics on caller input, the statistics hunt continued). Reference values cite scipy / statsmodels / mpmath / SymPy by the call that produced them. |
| `v04.rs` → `v04/v04_*.rs` | Feature tests written alongside the 0.4–0.6 APIs: `v04_polyhedron` (parametric polyhedron certificates and `PolyhedronProver`; the emitted Lean is pinned byte-for-byte to `fixtures/polyhedron_certificates.lean`, compiled against Mathlib with `linter.style.longLine` on), `v04_polytope` (exact and parametric polytopes: vertices, volume in any dimension, cuts, the bridge to the certificate search), `v04_sos` (sums of squares: the exact Gram pipeline, facial reduction on random sums of squares, refutation, Motzkin stays `Unknown`; Lean pinned to `fixtures/sos_certificates.lean`). |
...
| `fixtures/*.json`, `fixtures/*.lean` | Oracle data (JSON, regenerated by `scripts/*.py`, < 2 MB total) and the two Mathlib-compiled Lean pins of `v04`. Modules reach them with `include_str!("../fixtures/…")`. |
