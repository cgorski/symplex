...
fn residue_does_not_panic_on_hard_case() {
    ...
    let __vars_ctx = symplex::default_context().clone(); symplex::syms!(__vars_ctx; x);
    let f = &symplex::default_context().int(1) / &x.powi(2);
    // We just verify no panic; the result may be Err or an infinite expression.
    let _result = f.residue(&x, &symplex::default_context().int(0));
}
...
fn fourier_of_constant() {
    ...
    if let Ok(v) = result.subs(&x, &symplex::default_context().rational(1, 2)).eval_f64() {
        assert!(
            (v - 1.0).abs() < 0.1,
...
fn fourier_of_constant_at_zero() {
    ...
    if let Ok(v) = result.subs(&x, &symplex::default_context().int(0)).eval_f64() {
        assert!(
            (v - 1.0).abs() < 0.1,
