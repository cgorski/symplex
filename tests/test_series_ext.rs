...
fn residue_1_over_x_minus_1() {
    ...
    // Res(1/(x-1), x=1) = 1
    let __vars_ctx = __ctx.clone(); symplex::syms!(__vars_ctx; x);
    let f = &__ctx.int(1) / &(&x - 1);
    let r = f.residue(&x, &__ctx.int(1));
...
fn fourier_of_constant() {
    ...
    if let Ok(v) = result.subs(&x, &__ctx.rational(1, 2)).eval_f64() {
        assert!(
            (v - 1.0).abs() < 0.1,
...
fn fourier_of_constant_at_zero() {
    ...
    if let Ok(v) = result.subs(&x, &__ctx.int(0)).eval_f64() {
        assert!(
            (v - 1.0).abs() < 0.1,
