...
fn gamma_at_integers() {
    ...
    assert_eq!(format!("{}", symplex::int(1).gamma().eval()), "1");
    assert_eq!(format!("{}", symplex::int(2).gamma().eval()), "1");
    assert_eq!(format!("{}", symplex::int(3).gamma().eval()), "2");
    assert_eq!(format!("{}", symplex::int(5).gamma().eval()), "24");
    assert_eq!(format!("{}", symplex::int(7).gamma().eval()), "720");
}
...
fn gamma_via_macro() {
    vars!(x);
    let g = expr!(gamma(x));
    let result = g.subs(&x, &symplex::int(5)).eval();
...
fn log_gamma_at_integers() {
    ...
    let result = symplex::int(1).log_gamma().eval();
    let s = format!("{result}");
    assert_eq!(s, "0", "LogGamma(1) should be 0, got: {s}");

    // LogGamma(2) = ln(1!) = ln(1) = 0
    let result2 = symplex::int(2).log_gamma().eval();
...
fn log_gamma_at_larger_integer() {
    ...
    let result = symplex::int(5).log_gamma().eval();
    let s = format!("{result}");
    assert!(
...
#[test]
fn erf_at_zero() {
    assert_eq!(format!("{}", symplex::int(0).erf().eval()), "0");
}
...
fn erf_via_macro() {
    vars!(x);
    let result = expr!(erf(x)).subs(&x, &symplex::int(0)).eval();
...
fn erf_nonzero_stays_symbolic() {
    ...
    let result = symplex::int(1).erf().eval();
    let s = format!("{result}");
...
#[test]
fn erfc_at_zero() {
    assert_eq!(format!("{}", symplex::int(0).erfc().eval()), "1");
}
...
fn erfc_via_macro() {
    vars!(x);
    let result = expr!(erfc(x)).subs(&x, &symplex::int(0)).eval();
    assert_eq!(format!("{result}"), "1");
}

#[test]
fn erfc_nonzero_stays_symbolic() {
    let result = symplex::int(1).erfc().eval();
    let s = format!("{result}");
    assert!(
        s.contains("erfc"),
...
fn beta_symmetry() {
    ...
    let b23 = symplex::int(2).beta(&symplex::int(3)).eval();
    let b32 = symplex::int(3).beta(&symplex::int(2)).eval();
...
fn beta_ones() {
    ...
    let result = symplex::int(1).beta(&symplex::int(1)).eval();
    assert_eq!(format!("{result}"), "1");
}
...
fn beta_larger_values() {
    ...
    let result = symplex::int(3).beta(&symplex::int(4)).eval();
