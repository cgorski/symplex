...
fn evalf_gamma_1_is_one() {
    // Γ(1) = 0! = 1
    let result = symplex::int(1).gamma().eval_decimal(30).unwrap();
...
fn evalf_gamma_5_is_24() {
    ...
    let result = symplex::int(5).gamma().eval_decimal(50).unwrap();
    assert!(
...
fn evalf_gamma_10_is_362880() {
    // Γ(10) = 9! = 362880
    let result = symplex::int(10).gamma().eval_decimal(30).unwrap();
    let val: f64 = result.parse().unwrap();
    assert!(
...
fn evalf_gamma_neg_integer_is_pole() {
    ...
    let result2 = symplex::int(-5).gamma().eval_decimal(15);
