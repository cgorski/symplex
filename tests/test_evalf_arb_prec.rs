...
fn evalf_gamma_3_5_high_prec() {
    ...
    let result = val.gamma().evalf(30).unwrap();
    assert!(
...
fn evalf_gamma_1_is_one() {
    // Γ(1) = 0! = 1
    let result = symplex::int(1).gamma().evalf(30).unwrap();
...
fn evalf_gamma_5_is_24() {
    ...
    let result = symplex::int(5).gamma().evalf(50).unwrap();
    assert!(
...
fn evalf_gamma_10_is_362880() {
    // Γ(10) = 9! = 362880
    let result = symplex::int(10).gamma().evalf(30).unwrap();
    let val: f64 = result.parse().unwrap();
    assert!(
...
fn gamma_half_digits_increase_monotonically() {
    ...
    let half = symplex::rational(1, 2);
    let r15 = half.gamma().evalf(15).unwrap();
    let r30 = half.gamma().evalf(30).unwrap();
    let r50 = half.gamma().evalf(50).unwrap();
...
fn gamma_integer_same_at_multiple_precisions() {
    ...
    let r10 = five.gamma().evalf(10).unwrap();
    let r30 = five.gamma().evalf(30).unwrap();
    let r50 = five.gamma().evalf(50).unwrap();
...
fn evalf_gamma_neg_integer_is_pole() {
    ...
    let result2 = symplex::int(-5).gamma().evalf(15);
...
fn gamma_reflection_identity() {
    ...
    let z = symplex::rational(1, 3);
    let gz = z.gamma().evalf_f64().unwrap();
    let one_minus_z = symplex::rational(2, 3);
    let g1mz = one_minus_z.gamma().evalf_f64().unwrap();
    let product = gz * g1mz;
