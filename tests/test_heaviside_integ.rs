...
fn heaviside_positive_coeff_ftc() {
    ...
    symplex::vars!(x);
    let inner = &symplex::int(2) * &x - &symplex::int(1);
...
fn heaviside_negative_coeff_ftc() {
    ...
    symplex::vars!(x);
    let inner = &symplex::int(-2) * &x + &symplex::int(3);
    let heaviside = inner.heaviside();
    let antideriv = heaviside.integrate(&x);
...
fn heaviside_linear_integration_not_unevaluated() {
    ...
    symplex::vars!(x);
    let inner = &symplex::int(3) * &x + &symplex::int(2);
...
fn dirac_negative_coeff() {
    // ∫ δ(-3x + 6) dx = H(-3x+6) / 3
    symplex::vars!(x);
    let inner = &symplex::int(-3) * &x + &symplex::int(6);
...
fn dirac_positive_coeff() {
    // ∫ δ(4x - 8) dx = H(4x-8) / 4
    symplex::vars!(x);
    let inner = &symplex::int(4) * &x - &symplex::int(8);
    let delta = inner.dirac_delta();
    let result = delta.integrate(&x);
    let display = format!("{}", result);
    assert!(
