...
fn photon_energy_e_equals_hf() {
    ...
    let f_qty: Qty<FrequencyDim> = Qty::from_ex(__ctx.symbol("f"));
...
fn thermal_energy_kb_t() {
    ...
    let t_qty = Qty::<TemperatureDim>::from_ex(__ctx.symbol("T"));
...
fn gravitational_force() {
    ...
    let __vars_ctx = __ctx.clone(); symplex::syms!(__vars_ctx; m1, m2, r);
